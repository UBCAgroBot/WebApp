...
const App = () => {
  ...
  const [output, setOutput] = useState(['Waiting for WebSocket connection...'])
  ...
  useEffect(() => {
    ...
    socketRef.current.onclose = () => {
      if (socketRef.current && socketRef.current.readyState === WebSocket.CLOSED && initialConnection) {
    ...
    <div className='grid lg:h-screen w-full p-4 gap-4 lg:grid-cols-[3fr,2fr]'>
      ...
      <div className='flex flex-col gap-4 h-screen lg:h-full overflow-auto'>
        <div className='flex flex-col gap-2 border p-4 rounded-lg shadow h-full'>
          ...
          <div
            className='bg-black rounded-lg p-4 text-green-400 dark:text-green-500 h-full overflow-auto'
